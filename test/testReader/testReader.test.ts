...
test('Create a Xray JSON import file', async () => {
  ...
  const xrayJsonExpected = JSON.parse(readFileSync('./test/testReader/importFiles/happyPath/xrayJsonExpected.json').toString());
  ...
  expect(JSON.stringify(xrayJson)).toBe(JSON.stringify(xrayJsonExpected));
});
...
test('Create a Xray JSON import file with imported and unimported test cases', async () => {
  const xtr = new PlayWrightTestReader(Mode.import);
  const data = readFileSync('./test/testReader/importFiles/importedTcAndUnimported/xray.testimporter.config.json');
  const config = JSON.parse(data.toString()) as ClientConfiguration;
  const result = await xtr.parseFiles(config);
